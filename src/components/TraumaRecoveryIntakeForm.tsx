...
const translations = {
  en: {
    ...
    eligibilityLink: "contact our Crisis Counseling team",
    age18: "I am 18 years old or older",
    huskyInsurance: "I am on a HUSKY insurance plan",
  ...
  es: {
    ...
    eligibilityLink: "contacte a nuestro equipo de Consejería de Crisis",
    age18: "Tengo 18 años o más",
    huskyInsurance: "Tengo un plan de seguro HUSKY",
...
interface TraumaRecoveryIntakeFormProps {
  language?: "en" | "es";
}

export const TraumaRecoveryIntakeForm = ({ language = "en" }: TraumaRecoveryIntakeFormProps) => {
  ...
  const onSubmit = async (data: z.infer<ReturnType<typeof createSchema>>) => {
    ...
    try {
      const { error: supabaseError } = await supabase.from("trauma_recovery_intake").insert({
        ...
        is_18_or_older: data.is18OrOlder,
        has_husky_insurance: data.hasHuskyInsurance,
          ...
          <div className="space-y-3 bg-accent/10 p-4 rounded-lg">
            ...
            <div className="space-y-3">
              <div className="flex items-start space-x-2">
                <Checkbox
                  id="is18OrOlder"
                  checked={form.watch("is18OrOlder")}
                  onCheckedChange={(checked) => form.setValue("is18OrOlder", checked as boolean)}
                />
                <Label htmlFor="is18OrOlder" className="font-normal cursor-pointer">
                  {t.age18}
                </Label>
              </div>
              <div className="flex items-start space-x-2">
                <Checkbox
                  id="hasHuskyInsurance"
                  checked={form.watch("hasHuskyInsurance")}
                  onCheckedChange={(checked) => form.setValue("hasHuskyInsurance", checked as boolean)}
                />
                <Label htmlFor="hasHuskyInsurance" className="font-normal cursor-pointer">
                  {t.huskyInsurance}
                </Label>
              </div>
            </div>
            {(form.formState.errors.is18OrOlder || form.formState.errors.hasHuskyInsurance) && (
              <p className="text-sm text-destructive">{t.eligibilityRequired}</p>
            )}
          </div>
